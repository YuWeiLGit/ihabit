...
class CalendarFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        calendarView.setOnDateChangeListener { _, year: Int, month: Int, dayOfMonth: Int ->
            var rMonth=""
            if(month+1<10){
                rMonth="0"+(month+1).toString()
            }
            var rDayOfMonth=dayOfMonth.toString()
            if(dayOfMonth<10){
                rDayOfMonth= "0$dayOfMonth"
            }
            selectDate = "$year-$rMonth-$rDayOfMonth"
            date = selectDate
        }
        ...
        back.setOnClickListener {
            ...
            if(activity is AddHabitActivity){
                (activity as AddHabitActivity).sendDateString(selectDate)
            }
